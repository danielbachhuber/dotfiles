...
function useListing() {
  ...
  const load = useCallback(async () => {
    setListing((await rpc.call("listReviews", null)) as Listing);
  }, [rpc]);
...
function Panel() {
  ...
  const onArchive = useCallback(
    (row: Row) => {
      void (async () => {
        const result = await rpc.call("archiveReviewThread", { repo: row.repo, number: row.number });
  ...
  const onRefresh = useCallback(async () => {
    setBusy(true);
    try {
      const result = await rpc.call("refreshReviews", null);
...
function OpenPullRequest({ threadId, isCompactViewport }: PluginThreadHeaderActionProps) {
  ...
  useEffect(() => {
    let live = true;
    void (async () => {
      const result = await rpc.call("reviewForThread", { threadId });
...
export { Panel as ReviewsPanel, NeedsReviewCount, OpenPullRequest as OpenReviewedPullRequest };
