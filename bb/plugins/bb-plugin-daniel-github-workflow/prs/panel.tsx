...
function useListing() {
  ...
  const load = useCallback(async () => {
    setListing((await rpc.call("listPullRequests", null)) as Listing);
  }, [rpc]);
...
function Panel() {
  ...
  const onWork = useCallback(
    (row: Row) => {
      ...
      void (async () => {
        try {
          const result = await rpc.call("workOnPullRequest", { repo: row.repo, number: row.number });
          if (result.threadId) {
  ...
  const onArchive = useCallback(
    (row: Row) => {
      void (async () => {
        const result = await rpc.call("archivePullRequestThread", {
          repo: row.repo,
          number: row.number,
        });
  ...
  const onRefresh = useCallback(async () => {
    setBusy(true);
    try {
      const result = await rpc.call("refreshPullRequests", null);
...
export { Panel as PullRequestsPanel, NeedsActionCount, OpenPullRequest };
