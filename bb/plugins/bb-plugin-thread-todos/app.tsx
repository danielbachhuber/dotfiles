...
import { countsFor, headerLabel, orderForDisplay, rowStatusFor } from "./todos/list.js";
...
/**
 * The "To-do" tab in bb's thread panel. Checkboxes on the left, an add field
 * pinned to the bottom so it stays reachable in a long list.
 */
...
function TodoCount({ threadId, isCompactViewport }: PluginThreadHeaderActionProps) {
  ...
  const label = useMemo(
    () => (todos ? headerLabel(countsFor(threadId, todos)) : null),
    [threadId, todos],
  );
