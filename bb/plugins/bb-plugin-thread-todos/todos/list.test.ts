...
describe("countsFor and its labels", () => {
  ...
  it("says All done once the list is finished", () => {
    expect(headerLabel({ threadId: "t1", open: 0, done: 3 })).toBe("All done");
  });

  it("counts what is left, not what is finished", () => {
    expect(headerLabel({ threadId: "t1", open: 2, done: 7 })).toBe("2 left");
  });
...
describe("renderForAgent", () => {
  ...
  it("says so when the list is empty", () => {
    expect(renderForAgent([])).toBe("The to-do list is empty.");
  });
});
